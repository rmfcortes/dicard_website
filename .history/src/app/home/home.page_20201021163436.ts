...
export class HomePage implements OnInit, AfterContentInit{
  ...
  setForm() {
    this.form = new FormGroup({
      ...
      'phone': new FormControl('', Validators.compose([Validators.minLength(10), Validators.pattern(/^-?(0|[1-9]\d*)?$/)])),
    ...
    this.validation_messages = {
      'email': [
        ...
        'phone': [
          { type: 'minlength', message: 'El número debe ser de 10 dígitos' },
          { type: 'pattern', message: 'Teléfono inválido' },
        ],
        'msg': [
          { type: 'required', message: 'Este campo es requerido' },
        ],
  ...
  imgLoad(el, hideEl: HTMLElement) {
    hideEl.classList.add('notDisplay')
    el.originalTarget.classList.remove('hidden')
    this.animationService.zoomIn(el.originalTarget)
  }
